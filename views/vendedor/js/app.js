...
(() => {
  ...
  function legacyNotify(title, message, type = 'info', duration = 4000) {
    ...
    cart.forEach(item => {
      ...
      li.innerHTML = `
        <div style="flex:1">
          ...
          <div class="muted" style="font-size:12px">${formatBs(p.precio)} × ${item.qty}</div>
        </div>
        <div class="qty">
  ...
  function checkout(){
    if(cart.length === 0) {
      legacyNotify('Carrito', 'El carrito está vacío', 'warning');
      return;
    }
    ...
    renderClientes();
    renderKpis();
    legacyNotify('Pedido registrado', `Pedido registrado: ${id}`, 'success');
  }
