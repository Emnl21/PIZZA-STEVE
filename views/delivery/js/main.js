...
// Confirmación de logout del repartidor usa diálogo nativo del navegador.
...
// Almacenar sucursales y sus marcadores
let sucursales = [];
let sucursalMarkers = [];
...
function findNearestBranch(userLat, userLng) {
    ...
    sucursales.forEach(branch => {
        if (branch.latitud && branch.longitud && branch.activa) {
            const distance = calculateDistance(userLat, userLng, branch.latitud, branch.longitud);
            if (distance < minDistance) {
                minDistance = distance;
                nearestBranch = { ...branch, distance: distance };
            }
        }
    });
...
async function loadBranches() {
    try {
        ...
        sucursalMarkers.forEach(marker => map.removeLayer(marker));
        sucursalMarkers = [];
        
        // Agregar marcadores para cada sucursal activa con coordenadas
        sucursales.forEach(branch => {
            if (branch.latitud && branch.longitud && branch.activa) {
                const marker = L.marker([branch.latitud, branch.longitud], {
                    icon: L.icon({
                ...
                const popupContent = `
                    <b>📍 ${branch.nombre}</b><br>
                    ${branch.direccion || ''}<br>
                    ${branch.telefono ? `<i class="bi bi-telephone"></i> ${branch.telefono}<br>` : ''}
                    ${branch.horario_apertura && branch.horario_cierre ? 
                        `<small>Horario: ${branch.horario_apertura.substring(0,5)} - ${branch.horario_cierre.substring(0,5)}</small>` : ''}
                `;
        ...
        // Ajustar vista del mapa para mostrar todas las sucursales si hay alguna
        if (sucursalMarkers.length > 0) {
            const group = new L.featureGroup(sucursalMarkers);
            map.fitBounds(group.getBounds().pad(0.1));
        }
    } catch (error) {
        console.error('Error loading branches:', error);
        notifyDelivery('Error al cargar las sucursales: ' + error.message, 'warning');
    }
}
...
function renderDashboard() {
    ...
    let pending = 0;
    let earnings = 0;
    const payments = { efectivo: 0, qr: 0 };
...
function clearMap() {
    ...
    // Eliminar solo los marcadores de pedidos (rojos y verdes), no los de sucursales (azules)
    map.eachLayer((layer) => {
        if (layer instanceof L.Marker) {
            const latLng = layer.getLatLng();
            // Verificar si no es un marcador de sucursal
            const isBranchMarker = sucursalMarkers.some(m => 
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Cargar sucursales y datos si la sesión es válida
    await loadBranches();
    await loadDataFromAPI();
